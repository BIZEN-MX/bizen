...
export function LessonEngine({ lessonSteps, onComplete, onExit, onProgressChange, isRepeat }: LessonEngineProps) {
  ...
  const handleContinue = useCallback(() => {
    ...
    if (!currentStep) {
      console.warn("[LessonEngine] No current step in handleContinue");
      return
    }

    // console.log("[LessonEngine] handleContinue triggered", { isSummaryStep, isLastStep, isContinueEnabled: state.isContinueEnabled });

    // For Summary Step, handle its two phases (celebration -> stats)
    if (isSummaryStep) {
      if (!state.isContinueEnabled) {
        // console.log("[LessonEngine] Summary phase 1 -> phase 2");
        haptic.light()
        dispatch({ type: "TRIGGER_ACTION" })
      } else {
        // console.log("[LessonEngine] Summary phase 2 -> onExit");
        onExit?.()
      }
      return
    }
    ...
    if ((nextStepIsSummary || isAtEndOfOriginals) && !state.hasBuiltReviewSteps) {
      if (state.incorrectSteps.length > 0) {
        ...
        // Find if summary exists in allSteps (it might be there but index is not next)
        const summaryIdx = state.allSteps.findIndex(s => s.stepType === "summary");
        if (summaryIdx >= 0 && summaryIdx !== state.currentStepIndex) {
          dispatch({ type: "GO_TO_SUMMARY" })
        } else {
      ...
      >
        ...
        {showRecallButton && (
          ...
          <div className="recall-widget-container" style={{
            ...
            <button
              className="group flex items-center gap-[clamp(6px,1vw,10px)] px-[clamp(16px,2.5vw,24px)] py-[clamp(10px,1.5vw,14px)] bg-white/90 backdrop-blur-md rounded-full border border-blue-200/60 shadow-[0_8px_32px_rgba(37,99,235,0.15)] text-blue-700 font-bold text-[clamp(13px,1.8vw,16px)] whitespace-nowrap outline-none cursor-pointer transition-all duration-200 hover:bg-blue-50 hover:border-blue-300 hover:shadow-[0_12px_40px_rgba(37,99,235,0.25)] hover:-translate-y-0.5 active:scale-[0.97] active:translate-y-0"
            ...
            >
              ...
              >
                <path d="M6 2H18C19.1046 2 20 2.89543 20 4V20C20 21.1046 19.1046 22 18 22H6C4.89543 22 4 21.1046 4 20V4C4 2.89543 4.89543 2 6 2Z" stroke="currentColor" strokeWidth="2.5" strokeLinejoin="round" />
                ...
                <path d="M6 2V22" stroke="currentColor" strokeWidth="2" opacity="0.4" />
              </svg>
        ...
        {showRecallOverlay && lastInfoStep && (
          <div
            className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-[9000] flex items-center justify-center p-[24px_16px]"
            onClick={() => { playFlipSound(); setShowRecallOverlay(false) }}
            style={{ animation: "recallFadeIn 0.2s ease" }}
          >
            <div
              onClick={(e) => e.stopPropagation()}
              className="bg-white rounded-[28px] w-full max-w-[640px] max-h-[80vh] overflow-y-auto relative border-2 border-blue-500/20 shadow-[0_0_0_4px_rgba(59,130,246,0.06),0_2px_6px_rgba(59,130,246,0.08),0_10px_28px_rgba(59,130,246,0.14),0_32px_64px_rgba(15,23,42,0.25)]"
              style={{ animation: "recallSlideUp 0.25s cubic-bezier(0.4,0,0.2,1)" }}
            >
              {/* Shimmer top bar — synced with InfoStep */}
              <div className="h-[6px] bg-gradient-to-r from-blue-800 via-blue-600 to-blue-400 rounded-[26px_26px_0_0] relative overflow-hidden">
                <div 
                  className="absolute inset-0 w-1/2 bg-gradient-to-r from-transparent via-white/40 to-transparent" 
                  style={{ animation: "shimmerSlide 2.5s linear 1s infinite" }} 
                />
              </div>
              ...
              <button
                onClick={() => { playFlipSound(); setShowRecallOverlay(false) }}
                className="absolute top-[18px] right-[18px] w-[36px] h-[36px] rounded-full bg-blue-50 border-[1.5px] border-blue-200 cursor-pointer flex items-center justify-center text-blue-600 font-medium transition-opacity duration-200 hover:opacity-70"
              >
              ...
              {/* Content */}
              <div className="p-[32px_32px_40px]">

                {/* Key Concept Label */}
                <div className="inline-flex items-center gap-2 bg-blue-50 border-[1.5px] border-blue-200 px-3.5 py-1.5 rounded-full mb-6">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#2563eb" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20" />
                    <path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z" />
                  </svg>
                  <span className="text-[11px] font-medium text-blue-600 tracking-[0.06em] uppercase">Concepto Clave</span>
                </div>

                {(lastInfoStep as any).title && (
                  <h2 className="text-[clamp(20px,4vw,26px)] font-medium mb-4 leading-[1.2] bg-gradient-to-br from-slate-900 via-blue-800 to-blue-500 bg-clip-text text-transparent tracking-[-0.02em]">
                    {(lastInfoStep as any).title}
                  </h2>
                )}
                ...
                {(lastInfoStep as any).title && (
                  <div className="h-[2px] w-[200px] bg-gradient-to-r from-blue-500 via-blue-200 to-transparent rounded-full mb-5" />
                )}
                ...
                <div className="sticky bottom-0 left-0 right-0 h-[40px] bg-gradient-to-t from-blue-50/30 to-transparent pointer-events-none mt-4" />
              </div>
            </div>
          </div>
        )}
