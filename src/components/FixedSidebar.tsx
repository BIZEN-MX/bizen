                ...
                {user && isStudentOrGuest && !isAdminOrTeacher && (
                    <button
                        ...
                        style={{
                            ...
                            gap: 12,
                            padding: "14px 16px",
                            background: isCompactSidebar ? "transparent" : (pathname.startsWith("/bites") ? "rgba(239,68,68,0.12)" : "transparent"),
                            border: "none",
                            borderRadius: 10,
                            cursor: "pointer",
                            transition: "all 0.2s cubic-bezier(0.4, 0, 0.2, 1)",
                            fontSize: 14,
                            fontWeight: pathname.startsWith("/bites") ? 500 : 400,
                            textAlign: "left",
                            color: pathname.startsWith("/bites") ? "#ef4444" : "#64748B",
                            ...compactButtonOverrides(pathname.startsWith("/bites")),
                            position: "relative",
                            overflow: "hidden",
                            boxShadow: pathname.startsWith("/bites") ? "0 4px 12px rgba(239,68,68,0.12)" : "none"
                        }}
                        onMouseEnter={(e) => {
                            if (!isCompactSidebar) {
                                e.currentTarget.style.background = pathname.startsWith("/bites") ? "rgba(239,68,68,0.18)" : "#F1F5F9"
                                e.currentTarget.style.color = "#ef4444"
                            }
                        }}
                        onMouseLeave={(e) => {
                            if (!isCompactSidebar) {
                                e.currentTarget.style.background = pathname.startsWith("/bites") ? "rgba(239,68,68,0.12)" : "transparent"
                                e.currentTarget.style.color = pathname.startsWith("/bites") ? "#ef4444" : "#64748B"
                            }
                        }}
                    >
                        {pathname.startsWith("/bites") && (
                            <div style={{ position: "absolute", left: 0, top: "15%", height: "70%", width: "4px", backgroundColor: "#ef4444", borderRadius: "0 4px 4px 0" }} />
                        )}
                        <Play size={iconSize} strokeWidth={pathname.startsWith("/bites") ? 2.5 : 2} fill={pathname.startsWith("/bites") ? "#ef4444" : "none"} />
                        <span className="nav-item-label">BIZEN Bites</span>
                        <span style={{
                            marginLeft: "auto",
                            fontSize: 10,
                            fontWeight: 700,
