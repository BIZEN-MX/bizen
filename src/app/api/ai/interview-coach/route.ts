...
import { z } from 'zod';
import { interviewCoach, checkRateLimit } from '@/lib/lab/ai';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: jobData, error: jobError } = await supabase
      .from('lab_ai_jobs')
      .insert({ user_id: user.id, tool: 'interview-coach', input: validated, status: 'processing' })
      .select()
      .single();

    if (jobError) throw jobError;

    const result = await interviewCoach(validated);
