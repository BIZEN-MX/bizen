...
import { z } from 'zod';
import { pitchCoach, checkRateLimit } from '@/lib/lab/ai';

const InputSchema = z.object({
  pitchOutline: z.string().min(50),
  targetAudience: z.enum(['investors', 'accelerators', 'customers'])
});
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: jobData, error: jobError } = await supabase
      .from('lab_ai_jobs')
      .insert({ user_id: user.id, tool: 'pitch-coach', input: validated, status: 'processing' })
      .select()
      .single();

    if (jobError) throw jobError;

    const result = await pitchCoach(validated);
    ...
    console.error('Error in pitch-coach:', error);
