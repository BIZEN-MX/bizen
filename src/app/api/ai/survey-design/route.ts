...
import { z } from 'zod';
import { surveyDesign, checkRateLimit } from '@/lib/lab/ai';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: jobData, error: jobError } = await supabase
      .from('lab_ai_jobs')
      .insert({ user_id: user.id, tool: 'survey-design', input: validated, status: 'processing' })
      .select()
      .single();

    if (jobError) throw jobError;

    const result = await surveyDesign(validated);
    ...
    console.error('Error in survey-design:', error);
