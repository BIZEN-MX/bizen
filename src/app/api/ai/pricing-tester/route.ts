...
import { z } from 'zod';
import { pricingTester, checkRateLimit } from '@/lib/lab/ai';

const InputSchema = z.object({
  productFeatures: z.array(z.string()).min(1),
  targetSegment: z.string().min(5),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: jobData, error: jobError } = await supabase
      .from('lab_ai_jobs')
      .insert({ user_id: user.id, tool: 'pricing-tester', input: validated, status: 'processing' })
      .select()
      .single();

    if (jobError) throw jobError;

    const result = await pricingTester(validated);
    ...
    console.error('Error in pricing-tester:', error);
