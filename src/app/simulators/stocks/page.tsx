...
import {
  ...
  RefreshCw, CheckCircle2, Rocket, Activity, BarChart3
} from 'lucide-react'
...
export default function StockSimulatorPage() {
  ...
  if (loading) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', fontSize: 16, color: '#64748b' }}><RefreshCw size={20} style={{ marginRight: 10 }} /> Cargando...</div>
      ...
      <div className="sidebar-offset" style={{ minHeight: '100vh', background: 'linear-gradient(170deg,#f8fafc 0%,#f1f5f9 100%)', fontFamily: '-apple-system,BlinkMacSystemFont,"SF Pro Display","SF Pro Text",Helvetica,Arial,sans-serif' }}>
        <div style={{ maxWidth: 1180, margin: '0 auto', padding: 'clamp(24px,4vw,48px) clamp(16px,4vw,40px)', paddingBottom: 64 }}>
          ...
          <div style={{ marginBottom: 32, animation: 'fadeUp 0.5s ease' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.25)', borderRadius: 99, padding: '5px 14px', marginBottom: 14, fontSize: 12, fontWeight: 700, color: '#059669', letterSpacing: '0.06em', textTransform: 'uppercase' as const }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#10b981', display: 'inline-block' }} />
              Simulador Educativo — Sin dinero real
            </div>
            <h1 style={{ fontSize: 'clamp(26px,4.5vw,46px)', fontWeight: 800, margin: '0 0 10px', color: '#0f172a', letterSpacing: '-0.03em', lineHeight: 1.1 }}>Simulador de Mercado</h1>
          ...
          <div style={{ background: 'linear-gradient(135deg,#fffbeb,#fef3c7)', border: '1px solid #fde68a', borderRadius: 16, padding: '14px 20px', display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 13, color: '#92400e', lineHeight: 1.6, margin: 0 }}><strong>Precios de referencia (EOD).</strong> Las órdenes se ejecutan al cierre del siguiente día hábil. Este simulador NO usa dinero real. BIZEN no es un broker.</p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 180px), 1fr))', gap: 'clamp(10px, 2vw, 14px)', marginBottom: 32 }}>
            {[
              ...
              <div key={i} style={{ background: s.bg, borderRadius: 20, padding: '20px', border: '1.5px solid rgba(0,0,0,0.03)', boxShadow: '0 2px 8px rgba(0,0,0,0.02)' }}>
                <p style={{ fontSize: 10, fontWeight: 700, color: s.tc, textTransform: 'uppercase' as const, letterSpacing: '0.07em', margin: '0 0 6px', opacity: 0.7 }}>{s.label}</p>
                <p style={{ fontSize: 26, fontWeight: 800, color: s.tc, margin: '0 0 2px', letterSpacing: '-0.02em', lineHeight: 1.1 }}>{s.value}</p>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 24, overflowX: 'auto', paddingBottom: 4 }}>
            {tabs.map(t => {
              ...
              return (
                <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ border: active ? 'none' : '1px solid #e2e8f0', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, fontWeight: 600, fontSize: 14, borderRadius: 12, padding: '10px 18px', transition: 'all 0.2s', whiteSpace: 'nowrap', fontFamily: 'inherit', background: active ? '#0f172a' : 'white', color: active ? 'white' : '#64748b', boxShadow: active ? '0 4px 14px rgba(15,23,42,0.25)' : 'none' }}>
          ...
          <div style={{ background: 'white', borderRadius: 24, border: '1px solid #e2e8f0', boxShadow: '0 4px 24px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
            ...
            {activeTab === 'portfolio' && (
              <div style={{ padding: '28px 32px' }}>
                <h2 style={{ fontSize: 19, fontWeight: 700, color: '#0f172a', margin: '0 0 22px' }}>Mis Posiciones</h2>
                {!portfolio?.holdings?.length ? (
                  <div style={{ textAlign: 'center', padding: '60px 24px', background: '#f8fafc', borderRadius: 18, border: '2px dashed #e2e8f0' }}>
                    ...
                    <button onClick={() => setActiveTab('market')} style={{ padding: '11px 24px', background: 'linear-gradient(135deg,#10b981,#059669)', color: 'white', border: 'none', borderRadius: 12, fontWeight: 700, cursor: 'pointer', fontSize: 14, display: 'inline-flex', alignItems: 'center', gap: 8, fontFamily: 'inherit' }}>
                  ...
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead><tr style={{ borderBottom: '1px solid #f1f5f9' }}>{['Símbolo','Cantidad','Costo Prom.','Acción'].map(h => <th key={h} style={{ padding: '10px 14px', fontSize: 11, fontWeight: 700, color: '#94a3b8', textAlign: 'left', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>{h}</th>)}</tr></thead>
                      <tbody>{portfolio.holdings.map((h: any) => (
                        <tr key={h.id} className="sim-row-table" style={{ borderBottom: '1px solid #f8fafc' }}>
                          <td style={{ padding: '14px', fontWeight: 800, color: '#1e293b', fontSize: 15 }}>{h.symbol}</td>
            ...
            {activeTab === 'market' && (
              <div style={{ padding: '28px 32px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22, flexWrap: 'wrap', gap: 10 }}>
                  <h2 style={{ fontSize: 19, fontWeight: 700, color: '#0f172a', margin: 0 }}>ETFs & Acciones</h2>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 280px), 1fr))', gap: 'clamp(10px, 2vw, 14px)', marginBottom: 32 }}>
                  ...
                  {marketData.map(s => (
                    <div key={s.symbol} className="sim-stock-row" onClick={() => setOrderForm(f => ({ ...f, symbol: s.symbol }))}>
                      <div>
                        <p style={{ fontWeight: 800, fontSize: 16, color: '#0f172a', margin: '0 0 2px' }}>{s.symbol}</p>
                        ...
                        <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', background: '#e2e8f0', color: '#475569', borderRadius: 99, textTransform: 'uppercase' as const, letterSpacing: '0.05em' }}>{s.sector}</span>
                      </div>
                      <div style={{ textAlign: 'right' as const }}>
                        <p style={{ fontWeight: 800, fontSize: 18, color: '#0f172a', margin: '0 0 4px' }}>${s.price.toFixed(2)}</p>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, justifyContent: 'flex-end' }}>
                          {s.change >= 0 ? <ArrowUpRight size={14} color="#10b981" /> : <ArrowDownRight size={14} color="#ef4444" />}
                          <span style={{ fontSize: 13, fontWeight: 700, color: s.change >= 0 ? '#10b981' : '#ef4444' }}>{s.change >= 0 ? '+' : ''}{s.change}%</span>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div style={{ background: 'linear-gradient(135deg,#0f172a,#1e293b)', borderRadius: 24, padding: 'clamp(20px, 4vw, 32px)', color: 'white' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 26 }}>
                    ...
                    <h3 style={{ fontWeight: 700, fontSize: 18, margin: 0, color: 'white' }}>Colocar Orden</h3>
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 180px), 1fr))', gap: 20, marginBottom: 28 }}>
                    {/* Symbol */}
                    <div>
                      <label style={{ fontSize: 10, fontWeight: 700, color: '#64748b', textTransform: 'uppercase' as const, letterSpacing: '0.07em', display: 'block', marginBottom: 8 }}>Símbolo</label>
                      <select
                        value={orderForm.symbol}
                        onChange={e => setOrderForm(f => ({ ...f, symbol: e.target.value }))}
                        style={{ width: '100%', height: 48, padding: '0 14px', background: 'rgba(255,255,255,0.07)', border: '1.5px solid rgba(255,255,255,0.12)', borderRadius: 12, color: 'white', fontSize: 15, fontWeight: 700, fontFamily: 'inherit', outline: 'none', cursor: 'pointer', transition: 'border-color 0.2s' }}
                    ...
                    {/* Side */}
                    <div>
                      <label style={{ fontSize: 10, fontWeight: 700, color: '#64748b', textTransform: 'uppercase' as const, letterSpacing: '0.07em', display: 'block', marginBottom: 8 }}>Dirección</label>
                      <div style={{ display: 'flex', gap: 8, height: 48 }}>
                        {(['buy', 'sell'] as const).map(side => (
                          <button
                            ...
                            style={{
                              flex: 1, border: 'none', borderRadius: 12, fontWeight: 700, fontSize: 13,
                    ...
                    {/* Quantity */}
                    <div>
                      <label style={{ fontSize: 10, fontWeight: 700, color: '#64748b', textTransform: 'uppercase' as const, letterSpacing: '0.07em', display: 'block', marginBottom: 8 }}>Cantidad (acciones)</label>
                      <input
                        type="number" min={0.01} step={0.01} value={orderForm.qty}
                        onChange={e => setOrderForm(f => ({ ...f, qty: Number(e.target.value) }))}
                        style={{ width: '100%', boxSizing: 'border-box' as const, height: 48, padding: '0 14px', background: 'rgba(255,255,255,0.07)', border: '1.5px solid rgba(255,255,255,0.12)', borderRadius: 12, color: 'white', fontSize: 16, fontWeight: 800, fontFamily: 'inherit', outline: 'none', transition: 'border-color 0.2s, box-shadow 0.2s' }}
                  ...
                  <button
                    onClick={placeOrder}
                    disabled={placing}
                    style={{ padding: '14px 28px', background: placing ? '#334155' : 'linear-gradient(135deg,#10b981,#059669)', color: 'white', border: 'none', borderRadius: 12, fontWeight: 700, cursor: placing ? 'not-allowed' : 'pointer', fontSize: 15, fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 8, boxShadow: placing ? 'none' : '0 4px 14px rgba(16,185,129,0.35)', transition: 'all 0.2s' }}
            ...
            {activeTab === 'orders' && (
              <div style={{ padding: '28px 32px' }}>
                <h2 style={{ fontSize: 19, fontWeight: 700, color: '#0f172a', margin: '0 0 22px' }}>Historial de Órdenes</h2>
                {!portfolio?.orders?.length ? (
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead><tr style={{ borderBottom: '1px solid #f1f5f9' }}>{['Fecha','Símbolo','Tipo','Cant.','Estado'].map(h => <th key={h} style={{ padding: '10px 14px', fontSize: 11, fontWeight: 700, color: '#94a3b8', textAlign: 'left', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>{h}</th>)}</tr></thead>
                    <tbody>{portfolio.orders.map((o: any) => (
                      <tr key={o.id} className="sim-row-table" style={{ borderBottom: '1px solid #f8fafc' }}>
                        ...
                        <td style={{ padding: '14px', fontWeight: 800, color: '#1e293b' }}>{o.symbol}</td>
                        <td style={{ padding: '14px' }}><span style={{ padding: '4px 10px', borderRadius: 8, fontSize: 12, fontWeight: 700, background: o.side === 'buy' ? '#d1fae5' : '#fee2e2', color: o.side === 'buy' ? '#065f46' : '#991b1b' }}>{o.side === 'buy' ? '▲ Compra' : '▼ Venta'}</span></td>
                        <td style={{ padding: '14px', color: '#475569', fontWeight: 600 }}>{Number(o.quantity).toFixed(4)}</td>
                        <td style={{ padding: '14px' }}><span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 8, textTransform: 'uppercase' as const, background: o.status === 'pending' ? '#fef3c7' : o.status === 'filled' ? '#d1fae5' : '#f1f5f9', color: o.status === 'pending' ? '#92400e' : o.status === 'filled' ? '#065f46' : '#475569' }}>{o.status}</span></td>
                      </tr>
                    ))}</tbody>
                  </table>
                )}
            ...
            {activeTab === 'challenges' && (
              <div style={{ padding: '28px clamp(16px, 4vw, 32px)' }}>
                <h2 style={{ fontSize: 19, fontWeight: 700, color: '#0f172a', margin: '0 0 6px' }}>Misiones de Inversión</h2>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 300px), 1fr))', gap: 16 }}>
                  {CHALLENGES.map((c, i) => (
                    <div key={i} style={{ border: '1px solid #e2e8f0', borderRadius: 20, padding: '20px 22px', background: 'white', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', display: 'flex', gap: 14, alignItems: 'flex-start', transition: 'all 0.2s' }}>
                      ...
                      <div>
                        <p style={{ fontWeight: 700, fontSize: 15, color: '#0f172a', margin: '0 0 5px' }}>{c.title}</p>
                        <p style={{ fontSize: 13, color: '#64748b', lineHeight: 1.6, margin: '0 0 10px' }}>{c.desc}</p>
                        <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', background: 'linear-gradient(135deg,#ddd6fe,#c4b5fd)', color: '#4c1d95', borderRadius: 99, textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>{c.pts}</span>
                      </div>
                    </div>
                  ))}
                </div>
