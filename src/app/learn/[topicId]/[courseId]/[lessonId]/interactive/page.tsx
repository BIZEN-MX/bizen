...
function InteractiveLessonContent() {
  ...
  const handleExit = useCallback(() => {
    console.log("[InteractivePage] handleExit triggered", { topicNum, lessonIdStr, showTopicSplash });
    ...
    const allSlugs = subthemes ? subthemes.flatMap(s => (s.lessons || []).map((l: any) => l.slug)) : []
    
    console.log("[InteractivePage] Topic Completion Check:", { 
      allSlugsLength: allSlugs.length, 
      completedCount: (user?.user_metadata?.completedLessons || []).length 
    });
