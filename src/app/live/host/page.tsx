...
import { QUIZ_CATALOG, LiveQuiz } from "@/data/live-quizzes"
...
interface SaveTemplateState { title: string; isPublic: boolean }
interface QuizTemplate { id: string; title: string; description?: string; category: string; difficulty: number; question_count: number; times_used: number; is_public: boolean; created_at: string; owner_id?: string }
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      const res = await fetch("/api/live/templates", {
        ...
        body: JSON.stringify({ title: saveTitle, questions, category: "Custom", difficulty: 1, is_public: saveIsPublic })
      })
      if (res.ok) {
        setSaveDone(true)
        await loadTemplates()
        setTimeout(() => { setSaveModalOpen(false); setSaveDone(false); setSaveTitle(""); setSaveIsPublic(false) }, 1400)
      }
      ...
      <div style={{ minHeight: "100dvh", background: "linear-gradient(160deg, #060c1d 0%, #0a1628 60%, #050b18 100%)", padding: "0", marginLeft: sidebarOffset, transition: "margin-left 0.3s ease" }}>
        ...
        <div style={{ textAlign: "center", padding: "40px 24px 28px", animation: "fadeUp 0.35s ease" }}>
          ...
          <h1 style={{ margin: 0, fontSize: "clamp(26px, 4vw, 44px)", fontWeight: 500, color: "white", letterSpacing: "-0.03em", lineHeight: 1.1 }}>
            Bizen Live — Quiz en Tiempo Real
          </h1>
          <p style={{ margin: "12px auto 0", maxWidth: 500, fontSize: 15, color: "rgba(255,255,255,0.40)", lineHeight: 1.6 }}>
            Elige un quiz del catálogo, usa tus plantillas guardadas, o <strong style={{ color: "rgba(255,255,255,0.7)" }}>crea el tuyo propio</strong> y comparte el PIN para que todos se unan.
          </p>
        </div>
        ...
        {activeTab === "templates" && (
          <div style={{ padding: "0 40px 60px", maxWidth: 1400, margin: "0 auto" }}>
            {/* Create new button always visible at top of templates tab */}
            <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 24 }}>
              <button
                onClick={() => setHostStatus("create")}
                style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 20px", background: "linear-gradient(135deg, #0056E7, #1983FD)", border: "none", borderRadius: 12, color: "white", fontSize: 14, fontWeight: 600, cursor: "pointer", boxShadow: "0 4px 16px rgba(0,86,231,0.35)" }}
              >
                <IconPlus size={15} color="white" /> Crear nuevo quiz
              </button>
            </div>
            ...
            {templatesLoading ? (
              ...
              <div style={{ textAlign: "center", padding: "60px 24px" }}>
                <div style={{ width: 72, height: 72, borderRadius: 20, background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.2)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
                  <IconEdit size={32} color="#818cf8" />
                </div>
                <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 16, margin: "0 0 8px" }}>Aún no tienes quizes guardados</p>
                <p style={{ color: "rgba(255,255,255,0.25)", fontSize: 14, margin: "0 0 24px" }}>Crea tu primer quiz personalizado y guárdalo para reutilizarlo cuando quieras.</p>
                <button onClick={() => setHostStatus("create")} style={{ padding: "12px 24px", background: "linear-gradient(135deg, #0d2a6b, #1a56db)", border: "none", borderRadius: 14, color: "white", fontSize: 14, fontWeight: 600, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 8, boxShadow: "0 4px 16px rgba(15,98,254,0.35)" }}>
                  <IconPlus size={15} color="white" /> Crear mi primer quiz
                </button>
              </div>
            ) : (
                  ...
                  <div key={tpl.id} className="tpl-card" style={{ animationDelay: `${idx * 0.06}s`, borderRadius: 20, background: "rgba(255,255,255,0.04)", border: "1.5px solid rgba(99,102,241,0.2)", overflow: "hidden", boxShadow: "0 4px 20px rgba(0,0,0,0.25)" }}>
                    ...
                    <div style={{ background: "linear-gradient(135deg, rgba(55,48,163,0.6), rgba(99,102,241,0.4))", padding: "18px 20px 14px", borderBottom: "1px solid rgba(99,102,241,0.15)" }}>
                      ...
                      <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                        ...
                        {tpl.owner_id !== user?.id && <span style={{ background: "rgba(251,191,36,0.1)", color: "#fbbf24", padding: "2px 10px", borderRadius: 99, fontSize: 11 }}>Comunidad</span>}
                      </div>
                    </div>
                    {/* Body */}
      ...
      <div style={{ minHeight: "100dvh", background: "#060c1d", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 40, textAlign: "center", marginLeft: sidebarOffset, transition: "margin-left 0.3s ease" }}>
        ...
        {saveModalOpen && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: 24 }} onClick={() => setSaveModalOpen(false)}>
            <div style={{ background: "#0d1b36", border: "1.5px solid rgba(99,102,241,0.3)", borderRadius: 24, padding: "32px 28px", width: "100%", maxWidth: 440, boxShadow: "0 24px 64px rgba(0,0,0,0.6)" }} onClick={e => e.stopPropagation()}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
                ...
                <div>
                  <h3 style={{ margin: 0, color: "white", fontSize: 18, fontWeight: 600 }}>Guardar en Mis Quizes</h3>
                  <p style={{ margin: 0, color: "rgba(255,255,255,0.4)", fontSize: 13 }}>{questions.length} preguntas · reutilizable</p>
                </div>
              </div>
              <label style={{ color: "rgba(255,255,255,0.5)", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", display: "block", marginBottom: 8 }}>Nombre del quiz</label>
              <input
                value={saveTitle}
                onChange={e => setSaveTitle(e.target.value)}
                placeholder="Ej. Quiz Semana 3 — Inversiones"
                style={{ width: "100%", padding: "13px 16px", background: "rgba(255,255,255,0.06)", border: "1.5px solid rgba(255,255,255,0.12)", borderRadius: 12, color: "white", fontSize: 15, outline: "none", boxSizing: "border-box", marginBottom: 16 }}
              />
              {/* Public toggle */}
              <button
                onClick={() => setSaveIsPublic(p => !p)}
                style={{ width: "100%", marginBottom: 20, padding: "12px 16px", background: saveIsPublic ? "rgba(16,185,129,0.1)" : "rgba(255,255,255,0.04)", border: `1.5px solid ${saveIsPublic ? "rgba(16,185,129,0.3)" : "rgba(255,255,255,0.1)"}`, borderRadius: 12, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, transition: "all 0.2s" }}
              >
                <div style={{ textAlign: "left" }}>
                  <div style={{ color: "white", fontWeight: 600, fontSize: 14 }}>{saveIsPublic ? "🌐 Quiz público" : "🔒 Quiz privado"}</div>
                  <div style={{ color: "rgba(255,255,255,0.4)", fontSize: 12, marginTop: 2 }}>{saveIsPublic ? "Otros usuarios de la comunidad podrán usarlo" : "Solo visible para ti"}</div>
                </div>
                <div style={{ width: 44, height: 24, borderRadius: 99, background: saveIsPublic ? "#10b981" : "rgba(255,255,255,0.15)", position: "relative", flexShrink: 0, transition: "all 0.2s" }}>
                  <div style={{ position: "absolute", top: 3, left: saveIsPublic ? 23 : 3, width: 18, height: 18, borderRadius: "50%", background: "white", transition: "all 0.2s", boxShadow: "0 1px 4px rgba(0,0,0,0.3)" }} />
                </div>
              </button>
              {saveDone ? (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "14px", borderRadius: 14, background: "rgba(16,185,129,0.12)", border: "1px solid rgba(16,185,129,0.3)", color: "#34d399", fontWeight: 600 }}>
                  <IconCheck size={18} /> ¡Quiz guardado en "Mis Plantillas"!
                </div>
              ) : (
                <div style={{ display: "flex", gap: 10 }}>
                  ...
                  <button onClick={handleSaveTemplate} disabled={!saveTitle.trim() || saveSaving} style={{ flex: 2, padding: "13px", background: saveTitle.trim() ? "linear-gradient(135deg, #0d2a6b, #1a56db)" : "rgba(255,255,255,0.08)", border: "none", borderRadius: 12, color: "white", cursor: saveTitle.trim() ? "pointer" : "not-allowed", fontWeight: 700, fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: saveTitle.trim() ? "0 4px 16px rgba(15,98,254,0.35)" : "none" }}>
                    {saveSaving ? "Guardando..." : <><IconEdit size={14} color="white" /> Guardar Quiz</>}
                  </button>
                </div>
              )}
            </div>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
          ...
          <button onClick={() => canLaunch && onLaunch(questions, title)} disabled={!canLaunch} style={{ flex: 2, padding: "14px", background: canLaunch ? "linear-gradient(135deg, #0056E7, #1983FD)" : "rgba(255,255,255,0.08)", border: "none", borderRadius: 14, color: "white", fontSize: 15, fontWeight: 800, cursor: canLaunch ? "pointer" : "not-allowed", boxShadow: canLaunch ? "0 8px 24px rgba(0,86,231,0.4)" : "none", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
            <IconBolt size={16} color="white" /> Previsualizar y lanzar
          </button>
        </div>
        <p style={{ textAlign: "center", color: "rgba(255,255,255,0.25)", fontSize: 12, margin: "12px 0 0" }}>
          Al finalizar la sesión podrás guardar este quiz en "Mis Plantillas" para reutilizarlo
        </p>
      </div>
    </div>
  )
