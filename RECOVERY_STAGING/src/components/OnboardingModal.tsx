...
export default function OnboardingModal({ onComplete }: OnboardingModalProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setActiveFeatureIdx((current) => (current + 1) % WALLET_FEATURES.length)
    }, 5000)
                ...
                <div className="space-y-4">
                  {isAnahuac && (
                    <div className="mb-4 flex justify-center w-full">
                      <div className="bg-white/5 p-3 rounded-2xl border border-white/10 shadow-lg inline-flex items-center justify-center">
                        <img src="/anahuac-logo.png" alt="Anáhuac" className="w-[72px] h-[72px] object-contain" />
                      </div>
                    </div>
                  )}
                  ...
                  <h1 className="text-4xl md:text-5xl font-black tracking-tight leading-[1.1]">
                    {isAnahuac ? (
                      <>Bienvenido León, <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">{profileName}</span></>
                    ) : (
                  ...
                  <p className="text-slate-400 text-lg max-w-md mx-auto leading-relaxed">
                    {isAnahuac ? "Bienvenido a tu plataforma de educación financiera Leones. En 2 minutos estarás listo." : "Bienvenido a la nueva era de la educación financiera. En 2 minutos estarás listo para dominar tus finanzas."}
                  </p>
                </div>
