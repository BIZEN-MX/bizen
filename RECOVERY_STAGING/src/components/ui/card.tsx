...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "relative rounded-3xl bg-white/95 border border-slate-200/60 shadow-md",
        "transition-all duration-300 ease-out",
        "hover:shadow-xl hover:translate-y-[-2px] hover:border-slate-300/80",
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none border-t border-white/40 rounded-3xl" />
      {props.children}
    </div>
  )
)
