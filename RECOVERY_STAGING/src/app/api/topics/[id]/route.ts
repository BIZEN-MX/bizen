...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      // 2. Fallback for ID mismatches (tema-01 <-> 1, or tema1 <-> tema-01)
      if (!topic) {
        // Extract numeric part from 'tema-01', 'tema1', or just '1'
        const numericMatch = id.match(/\d+/);
        if (numericMatch) {
          const num = numericMatch[0];
          const variations = [
            num,                          // '1'
            `tema-${num.padStart(2, '0')}`, // 'tema-01'
            `tema${parseInt(num)}`,       // 'tema1'
          ];

          for (const variant of variations) {
            if (variant === id) continue; // Already tried in primary lookup
