...
export function OnboardingProvider({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        // SUCCESSFUL FIX: Check BOTH Clerk metadata AND our database profile (dbProfile)
        // Also check localStorage as a bulletproof failsafe against DB connection errors on localhost
        const localComplete = typeof window !== 'undefined' ? localStorage.getItem('bizen_onboarding_finalized') === 'true' : false;
        
        // A user is complete if they have a real database profile with a nickname (meaning they passed onboarding)
        // OR if they hit the emergency localStorage fallback.
        const hasDbNickname = dbProfile && dbProfile.nickname;
        const isComplete = user.user_metadata?.onboarding_complete === true || hasDbNickname || localComplete;
        
        if (!isComplete && !showTour) {
            const timer = setTimeout(() => setShowSetup(true), 700)
            return () => clearTimeout(timer)
        } else if (isComplete && showSetup) {
            // Self-correction: if profile appeared while modal was open, close it
            setShowSetup(false)
        }
    }, [user, dbProfile, loading, pathname, showTour, showSetup])

    // Discovery mode (AUTO-EXPLORING) has been disabled as per user request
    /*
    useEffect(() => {
    ...
    }, [pathname, seenPaths, loading, user, showSetup, showTour, dbProfile])
    */

    const handleSetupComplete = useCallback(() => {
        // Enforce the bulletproof lock
        localStorage.setItem('bizen_onboarding_finalized', 'true')
